...
const branches = [
  { name:'فرع بورتسودان', tag:'المقر الرئيسي', address:'سوق بورتسودان الكبير، تقاطع البلدية والشرق، السودان.', phone:'+249 112 705 718', hours:'السبت - الخميس: 8:00 ص - 4:00 م', maps:'https://maps.app.goo.gl/Yh5tK4hNyY8USjhN7', embed:'https://maps.google.com/maps?q=Port%20Sudan,%20Sudan&t=&z=13&ie=UTF8&iwloc=&output=embed' },
...
<main>
  ...
  <section id="contact" class="contact section-pad"><div class="container contact-grid"><div class="contact-info"><span class="eyebrow light">${icon('message',16)} تواصل معنا اليوم</span><h2>لنبدأ حديثاً يصنع <i>الفرق.</i></h2><p>سواء كنت تخطط لرحلة، أو تحتاج إلى إجراءات موثوقة، أو تبحث عن فرصتك العقارية القادمة — فريقنا جاهز لخدمتك.</p><div class="contact-list"><a href="https://maps.app.goo.gl/BYkaZKm7cdKBqGDN6" target="_blank" rel="noreferrer"><span>${icon('map')}</span><div><small>العنوان الرئيسي</small><b>بورتسودان، جمهورية السودان</b></div>${icon('external',16)}</a><a class="numbers" href="tel:+249112705718"><span>${icon('phone')}</span><div><small>اتصل بفريق المبيعات</small><b dir="ltr">+249 112 705 718</b><b dir="ltr">+249 900 740 722</b></div></a><a href="mailto:contact@alharrazgroup.com"><span>${icon('mail')}</span><div><small>البريد الإلكتروني المعتمد</small><b dir="ltr">contact@alharrazgroup.com</b></div></a></div><a class="whatsapp-link" href="${whatsapp}" target="_blank" rel="noreferrer">${icon('message',20)} محادثة فورية على واتساب ${icon('arrow',17)}</a><div class="commitment">${icon('shield',25)}<p><strong>التزامنا لك</strong><span>خدمة آمنة وموثوقة، ورد أولي على طلبك خلال 24 ساعة.</span></p></div></div><div class="form-card"><div class="form-title"><span class="number-mark">01</span><div><h3>اطلب استشارتك الآن</h3><p>املأ البيانات التالية وسنصل إليك.</p></div></div>${form()}</div></div></section>
...
<footer class="footer"><div class="container footer-grid"><div>${logo(true)}<p class="footer-copy">نحوّل الإجراءات المعقدة إلى تجارب سهلة، ونبني علاقات طويلة الأمد قائمة على الثقة والجودة.</p><p class="registry">${icon('shield',17)} سجل تجاري معتمد رقم <b dir="ltr">2984</b></p></div><div><h4>خدماتنا</h4>${services.map(s=>`<button data-scroll="${s.id}">${s.title}</button>`).join('')}</div><div><h4>روابط سريعة</h4><button data-scroll="home">الرئيسية</button><button data-scroll="contact">طلب استشارة</button><button data-scroll="branches">مواقع الفروع</button><a href="mailto:contact@alharrazgroup.com">contact@alharrazgroup.com</a></div><div class="footer-contact"><h4>تواصل مباشر</h4><a href="tel:+249112705718" dir="ltr">${icon('phone',16)}+249 112 705 718</a><a href="${whatsapp}" target="_blank" rel="noreferrer">${icon('message',16)}واتساب خدمة العملاء</a><span>${icon('map',16)}بورتسودان، السودان</span></div></div><div class="container footer-bottom"><span>© ${new Date().getFullYear()} مجموعة الحراز للخدمات المتكاملة. جميع الحقوق محفوظة.</span><div><a href="#">سياسة الخصوصية</a><a href="#">الشروط والأحكام</a></div></div></footer>
